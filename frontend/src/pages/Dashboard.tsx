    ...
    <div style={{ background:'#050A12', color:'#F0EFE7', minHeight:'100vh', paddingTop:80 }}>
      ...
      <div style={{ borderBottom:'1px solid rgba(255,255,255,.06)', background:'rgba(255,255,255,.02)' }}>
        <div className="dash-head" style={{ maxWidth:1100, margin:'0 auto', padding:'40px 32px', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:20 }}>
          <div>
            <div style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.12em', textTransform:'uppercase', color:'#00FF87', marginBottom:10 }}>Dashboard</div>
      ...
      <div className="dash-body" style={{ maxWidth:1100, margin:'0 auto', padding:'48px 32px' }}>
        ...
        <div style={{ marginBottom:48 }}>
          ...
          {loading ? (
            <div style={{ textAlign:'center', padding:48, color:'rgba(240,239,231,.4)', fontFamily:'IBM Plex Sans, sans-serif' }}>Loading your bookings...</div>
          ) : upcoming.length === 0 ? (
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} style={{ background:'rgba(255,255,255,.02)', border:'1px dashed rgba(255,255,255,.08)', borderRadius:12, padding:'60px 32px', textAlign:'center' }}>
              ...
              <h3 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:20, marginBottom:10, color:'rgba(240,239,231,.6)' }}>No upcoming lessons</h3>
              <p style={{ color:'rgba(240,239,231,.35)', fontFamily:'IBM Plex Sans, sans-serif', marginBottom:28 }}>Book your first free demo and start the adventure!</p>
              ...
              {upcoming.map((b, i) => (
                <motion.div key={b.id} initial={{ y:20, opacity:0 }} animate={{ y:0, opacity:1 }} transition={{ delay: i*0.07 }} style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.07)', borderRadius:10, padding:'22px 28px', display:'flex', gap:20, alignItems:'center', flexWrap:'wrap' }}>
                  ...
                  <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
                    ...
                    <button onClick={() => setCancelId(b.id)} style={{ background:'rgba(248,113,113,.08)', border:'1px solid rgba(248,113,113,.2)', borderRadius:6, padding:'6px 14px', color:'#f87171', cursor:'pointer', fontSize:12, fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600 }}>Cancel</button>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        {past.length > 0 && (
          <div>
            <h2 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:18, marginBottom:20, color:'rgba(240,239,231,.6)' }}>Past Sessions</h2>
