  ...
  return (
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', marginBottom:8 }}>
        {tr.booking.dayNames.map(d => (
          <div key={d} style={{ textAlign:'center', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:11, color:'rgba(240,239,231,.3)', letterSpacing:'0.08em', padding:'8px 0' }}>{d}</div>
        ))}
      </div>
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                fontFamily:'IBM Plex Sans, sans-serif', fontWeight: isSel ? 700 : 400, fontSize:13,
...
export default function BookingPage({ setPage }: { setPage: (p: string) => void }) {
  ...
  const handleConfirm = async () => {
    ...
    if (isFreeDemo) {
      ...
      if (error) {
        if (error.code === '23505') toast.error(d.slotTaken)
      ...
      if (apptErr) {
        if (apptErr.code === '23505') toast.error(d.slotTaken)
      ...
      <div className="booking-inner" style={{ maxWidth:1000, margin:'0 auto', padding:'60px 32px', position:'relative', zIndex:1 }}>
        ...
        {step !== 4 && (
          <motion.div initial={{ y:32, opacity:0 }} animate={{ y:0, opacity:1 }} transition={{ duration:.7 }} style={{ textAlign:'center', marginBottom:56 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(0,255,135,.08)', border:'1px solid rgba(0,255,135,.2)', borderRadius:4, padding:'6px 16px', marginBottom:24, fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:11, letterSpacing:'0.12em', textTransform:'uppercase', color:'#00FF87' }}>
              <Wifi size={12} /> {d.badgeLive}
            </div>
            <h1 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:800, fontSize:'clamp(32px,5vw,52px)', lineHeight:1.05, letterSpacing:'-0.03em', marginBottom:16 }}>
              {d.bookTitle1}<span style={{ background:'linear-gradient(135deg,#00FF87,#60A5FA)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>{d.bookTitleHighlight}</span>
            </h1>
            <p style={{ color:'rgba(240,239,231,.55)', fontSize:17, fontFamily:'IBM Plex Sans, sans-serif', lineHeight:1.75 }}>
              {d.bookSubtitle}
            </p>
          </motion.div>
        )}
                ...
                <div key={n} style={{ display:'flex', alignItems:'center' }}>
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8 }}>
                    <motion.div
                      ...
                      style={{ width:36, height:36, borderRadius:'50%', display:'grid', placeItems:'center', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:14 }}
                    >
                      {step > n ? <CheckCircle size={16} /> : n}
                    </motion.div>
                    <span style={{ fontSize:11, fontFamily:'IBM Plex Sans, sans-serif', color: step >= n ? 'rgba(240,239,231,.7)' : 'rgba(240,239,231,.25)', whiteSpace:'nowrap', letterSpacing:'0.05em' }}>{label}</span>
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-24 }} transition={{ duration:.5 }}>
              ...
              {/* Paid courses */}
              <p style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.1em', color:'rgba(240,239,231,.4)', textTransform:'uppercase', marginBottom:14 }}>{d.chooseCourse}</p>

              {loadCourses ? (
                <div style={{ textAlign:'center', padding:40, color:'rgba(240,239,231,.4)', fontFamily:'IBM Plex Sans, sans-serif' }}>{d.loadingCourses}</div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                          <div style={{ background:'rgba(96,165,250,.12)', border:'1px solid rgba(96,165,250,.2)', borderRadius:6, padding:'4px 12px', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:11, color:'#60A5FA', letterSpacing:'0.08em' }}>
                            {c.category}
                          </div>
                          {isSel && <CheckCircle size={18} color="#60A5FA" />}
                        </div>
                        <div style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:17, color:'#F0EFE7', marginBottom:8 }}>{c.title}</div>
                        <div style={{ fontFamily:'IBM Plex Sans, sans-serif', fontSize:13, color:'rgba(240,239,231,.5)', lineHeight:1.6, marginBottom:16 }}>{c.description}</div>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                          <span style={{ fontFamily:'IBM Plex Sans, sans-serif', fontSize:12, color:'rgba(240,239,231,.4)' }}>Ages {c.age_min}–{c.age_max} · {c.duration_weeks}wks</span>
                          <span style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:800, fontSize:20, color:'#F0EFE7' }}>€{c.price_usd}</span>
                        </div>
                      </motion.button>
                    )
                  })}
              ...
              <div style={{ textAlign:'center' }}>
                <motion.button
                  ...
                  disabled={!selCourse}
                  style={{ background:'linear-gradient(135deg,#00FF87,#00D4AA)', color:'#050A12', border:'none', borderRadius:8, padding:'14px 40px', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:15, cursor: selCourse ? 'pointer' : 'not-allowed', display:'inline-flex', alignItems:'center', gap:8 }}
                >
                  {d.continueBtn} <ArrowRight size={16} />
                </motion.button>
              </div>
            </motion.div>
          )}
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-24 }} transition={{ duration:.5 }}>
              <button onClick={() => setStep(1)} style={{ background:'none', border:'none', color:'rgba(240,239,231,.45)', cursor:'pointer', display:'flex', alignItems:'center', gap:6, marginBottom:28, fontSize:14, fontFamily:'IBM Plex Sans, sans-serif' }}><ChevronLeft size={16} /> {d.back}</button>
              ...
              <div style={{ background: isFreeDemo ? 'rgba(0,255,135,.06)' : 'rgba(96,165,250,.06)', border: `1px solid ${isFreeDemo ? 'rgba(0,255,135,.2)' : 'rgba(96,165,250,.2)'}`, borderRadius:10, padding:'14px 20px', marginBottom:28, display:'flex', alignItems:'center', gap:12 }}>
                ...
                <span style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:15, color:'#F0EFE7' }}>{isFreeDemo ? d.freeDemoTitle : selCourse?.title}</span>
                <span style={{ marginLeft:'auto', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:800, fontSize:16, color: isFreeDemo ? '#00FF87' : '#F0EFE7' }}>
                  {isFreeDemo ? d.free : `€${selCourse?.price_usd}`}
                </span>
              </div>
              ...
              <div className="booking-cal-grid" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))', gap:24 }}>
                ...
                <div style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.07)', borderRadius:12, padding:28 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
                    ...
                    <span style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:16 }}>{format(month,'MMMM yyyy')}</span>
                  ...
                  <p style={{ fontSize:11, color:'rgba(240,239,231,.3)', marginTop:16, fontFamily:'IBM Plex Sans, sans-serif', textAlign:'center' }}>{d.weekdaysOnly}</p>
                </div>
                ...
                <div style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.07)', borderRadius:12, padding:28 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:24 }}>
                    ...
                    <span style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:16 }}>
                      {selDate ? format(selDate,'EEEE, MMM d') : d.selectDateFirst}
                    </span>
                  ...
                  {!selDate ? (
                    <div style={{ display:'grid', placeItems:'center', height:200, color:'rgba(240,239,231,.25)', fontFamily:'IBM Plex Sans, sans-serif', fontSize:14, textAlign:'center' }}>{d.pickDatePrompt.split('\n').map((line, i) => <span key={i}>{line}{i === 0 && <br />}</span>)}</div>
                  ) : (
                    <>
                    <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10 }}>
                      <Clock size={12} style={{ color:'rgba(0,255,135,.6)' }} />
                      <span style={{ fontFamily:'IBM Plex Sans, sans-serif', fontSize:12, color:'rgba(240,239,231,.4)' }}>
                        {d.berlinTime}
                      </span>
                    </div>
                  ...
                  <motion.button
                    ...
                    style={{ width:'100%', marginTop:24, background:'linear-gradient(135deg,#00FF87,#00D4AA)', color:'#050A12', border:'none', borderRadius:8, padding:'14px', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:15, cursor: selDate && selTime ? 'pointer' : 'not-allowed', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}
                  >
                    {d.continueBtn} <ArrowRight size={16} />
                  </motion.button>
                </div>
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-24 }} transition={{ duration:.5 }}>
              <div className="booking-confirm" style={{ maxWidth:520, margin:'0 auto', background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.07)', borderRadius:12, padding:40 }}>
                <button onClick={() => setStep(2)} style={{ background:'none', border:'none', color:'rgba(240,239,231,.45)', cursor:'pointer', display:'flex', alignItems:'center', gap:6, marginBottom:28, fontSize:14, fontFamily:'IBM Plex Sans, sans-serif' }}><ChevronLeft size={16} /> {d.back}</button>

                <h2 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:24, marginBottom:28 }}>
                  {isFreeDemo ? d.confirmFreeTitle : d.confirmPayTitle}
                </h2>
                ...
                <div style={{ background: isFreeDemo ? 'rgba(0,255,135,.06)' : 'rgba(96,165,250,.06)', border:`1px solid ${isFreeDemo ? 'rgba(0,255,135,.2)' : 'rgba(96,165,250,.2)'}`, borderRadius:10, padding:20, marginBottom:24 }}>
                  {[
                    { label: d.summarySession, value: isFreeDemo ? d.freeDemoTitle : selCourse?.title },
                    { label: d.summaryDate,    value: selDate && format(selDate,'EEEE, MMMM d, yyyy') },
                    { label: d.summaryTime,    value: selTime ? `${selTime} ${d.berlinTimeSuffix}` : '' },
                    { label: d.summaryPrice,   value: isFreeDemo ? d.free : `€${selCourse?.price_usd}` },
                  ].map(row => (
                    <div key={row.label} style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
                      <span style={{ color:'rgba(240,239,231,.5)', fontFamily:'IBM Plex Sans, sans-serif', fontSize:14 }}>{row.label}</span>
                      <span style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:14, color: row.label === 'Price' && isFreeDemo ? '#00FF87' : '#F0EFE7' }}>{row.value}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ marginBottom:16 }}>
                  <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>{d.childName}</label>
                  <input value={kidName} onChange={e => setKidName(e.target.value)} placeholder={d.childNamePh} style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'12px 16px', color:'#F0EFE7', fontFamily:'IBM Plex Sans, sans-serif', fontSize:15, outline:'none', boxSizing:'border-box' }} onFocus={e => e.target.style.borderColor='rgba(0,255,135,.5)'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,.1)'} />
                </div>

                <div style={{ marginBottom:16 }}>
                  <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>{d.childAge}</label>
                  <input type="number" min={5} max={17} value={kidAge} onChange={e => setKidAge(e.target.value)} placeholder={d.childAgePh} style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'12px 16px', color:'#F0EFE7', fontFamily:'IBM Plex Sans, sans-serif', fontSize:15, outline:'none', boxSizing:'border-box' }} onFocus={e => e.target.style.borderColor='rgba(0,255,135,.5)'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,.1)'} />
                </div>

                <div style={{ marginBottom:28 }}>
                  <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>{d.notesLabel}</label>
                  <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder={d.notesPh} rows={3} style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'12px 16px', color:'#F0EFE7', fontFamily:'IBM Plex Sans, sans-serif', fontSize:15, resize:'vertical', outline:'none', boxSizing:'border-box' }} onFocus={e => e.target.style.borderColor='rgba(0,255,135,.5)'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,.1)'} />
                </div>
                ...
                <button
                  ...
                  style={{ width:'100%', background:'linear-gradient(135deg,#00FF87,#00D4AA)', color:'#050A12', border:'none', borderRadius:8, padding:'16px', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:16, cursor: !user || loading ? 'not-allowed' : 'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:10, opacity: !user ? 0.5 : 1 }}
                >
                  {loading
                    ? (isFreeDemo ? d.confirming : d.redirecting)
                    : (isFreeDemo ? <><CheckCircle size={18} /> {d.confirmFreeBtn}</> : <>{d.proceedPayment} · €{selCourse?.price_usd}</>)
                  }
                </button>

                {!isFreeDemo && (
                  <p style={{ textAlign:'center', marginTop:14, fontSize:12, color:'rgba(240,239,231,.3)', fontFamily:'IBM Plex Sans, sans-serif' }}>
                    {d.stripeNote}
                  </p>
                )}
              </div>
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity:0, scale:.95 }} animate={{ opacity:1, scale:1 }} transition={{ duration:.6 }}>
              <div style={{ maxWidth:500, margin:'0 auto', textAlign:'center' }}>
                ...
                <h1 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:800, fontSize:40, marginBottom:16 }}>{d.bookedTitle}</h1>
                <p style={{ color:'rgba(240,239,231,.6)', fontFamily:'IBM Plex Sans, sans-serif', fontSize:17, lineHeight:1.75, marginBottom:32 }}>
                  {d.bookedDescPre}<strong style={{ color:'#F0EFE7' }}>{selDate && format(selDate,'MMMM d')}</strong>{d.bookedDescMid}<strong style={{ color:'#F0EFE7' }}>{selTime}</strong>{d.bookedDescPost}<br />{d.bookedEmail}
                </p>
                {booking?.zoom_link && (
                  <a href={booking.zoom_link} target="_blank" rel="noopener noreferrer" style={{ display:'inline-flex', alignItems:'center', gap:10, background:'rgba(37,99,235,.2)', border:'1px solid rgba(37,99,235,.4)', borderRadius:8, padding:'12px 24px', color:'#60A5FA', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, textDecoration:'none', marginBottom:28 }}>
                    <Video size={18} /> {d.joinZoom}
                  </a>
                )}
                <br />
                <button onClick={() => { setPage('dashboard'); window.scrollTo({ top:0 }) }} style={{ background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'12px 28px', color:'#F0EFE7', cursor:'pointer', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:14 }}>
                  {d.goToDashboard}
                </button>
              </div>
            </motion.div>
