      ...
      <div className="contact-head" style={{ padding:'80px 32px 60px', textAlign:'center', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:600, margin:'0 auto', position:'relative', zIndex:1 }}>
          <motion.div initial={{ y:32, opacity:0 }} animate={{ y:0, opacity:1 }} transition={{ duration:.7 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(0,255,135,.08)', border:'1px solid rgba(0,255,135,.2)', borderRadius:4, padding:'6px 16px', marginBottom:24, fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:11, letterSpacing:'0.12em', textTransform:'uppercase', color:'#00FF87' }}>Get in Touch</div>
            ...
            <p style={{ color:'rgba(240,239,231,.55)', fontSize:18, fontFamily:'IBM Plex Sans, sans-serif', lineHeight:1.75 }}>Questions about courses, scheduling, or anything else — we reply within hours.</p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="contact-body" style={{ maxWidth:1100, margin:'0 auto', padding:'0 32px 100px', display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))', gap:48, alignItems:'flex-start' }}>
        ...
        <motion.div initial={{ x:-40, opacity:0 }} animate={{ x:0, opacity:1 }} transition={{ duration:.7, delay:.1 }}>
          {[
            ...
            { icon:Phone,  color:'#60A5FA', label:'WhatsApp', value:'+49 (0) — via WhatsApp'     },
            { icon:MapPin, color:'#A78BFA', label:'Location', value:'Berlin, Germany 🇩🇪'         },
            { icon:Clock,  color:'#f59e0b', label:'Hours',    value:'Mon–Sat, 9am–8pm CET'       },
        ...
        <motion.div initial={{ x:40, opacity:0 }} animate={{ x:0, opacity:1 }} transition={{ duration:.7, delay:.2 }} style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.07)', borderRadius:12, padding:40 }}>
          {status === 'success' ? (
            <div style={{ textAlign:'center', padding:'40px 0' }}>
              ...
              <h3 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:24, marginBottom:12 }}>Message Sent!</h3>
              <p style={{ color:'rgba(240,239,231,.55)', fontFamily:'IBM Plex Sans, sans-serif', lineHeight:1.75, marginBottom:28 }}>We'll get back to you within a few hours.</p>
              <button onClick={() => { setStatus('idle'); setForm({ name:'', email:'', subject:'', message:'' }); }} style={{ background:'rgba(0,255,135,.1)', border:'1px solid rgba(0,255,135,.3)', borderRadius:6, padding:'10px 24px', color:'#00FF87', cursor:'pointer', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600 }}>Send Another</button>
            </div>
          ) : (
            <form onSubmit={submit}>
              <h2 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:22, marginBottom:28 }}>Send a Message</h2>
              ...
              <div style={{ marginBottom:28 }}>
                <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:11, letterSpacing:'0.1em', textTransform:'uppercase', color:'rgba(240,239,231,.5)', marginBottom:8 }}>Message</label>
