    ...
    <div style={{ minHeight:'100vh', background:'#050A12', display:'grid', placeItems:'center', padding:'100px 24px 60px', position:'relative' }}>
      ...
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} style={{ width:'100%', maxWidth:440, position:'relative', zIndex:1 }}>
          ...
          {mode !== 'forgot' && (
            <>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
                ...
                <span style={{ fontSize:12, color:'rgba(240,239,231,.35)', fontFamily:'IBM Plex Sans, sans-serif' }}>or continue with email</span>
                <div style={{ flex:1, height:1, background:'rgba(255,255,255,.1)' }} />
              </div>
            </>
          )}
          ...
          <form onSubmit={handleSubmit}>
            {mode === 'signup' && (
              <div style={{ marginBottom:18 }}>
                <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>Full Name</label>
            ...
            <div style={{ marginBottom:18 }}>
              <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>Email Address</label>
              <div style={{ position:'relative' }}>
            ...
            {mode !== 'forgot' && (
              <div style={{ marginBottom: mode === 'login' ? 8 : 28 }}>
                <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>Password</label>
                <div style={{ position:'relative' }}>
            ...
            {mode === 'login' && (
              <div style={{ textAlign:'right', marginBottom:24 }}>
                <button type="button" onClick={() => setMode('forgot')} style={{ background:'none', border:'none', color:'rgba(0,255,135,.7)', cursor:'pointer', fontFamily:'IBM Plex Sans, sans-serif', fontSize:13 }}>Forgot password?</button>
              </div>
            )}
        ...
        <div style={{ textAlign:'center', marginTop:24 }}>
          <button onClick={() => { setPage('home'); window.scrollTo({ top:0 }); }} style={{ background:'none', border:'none', color:'rgba(240,239,231,.3)', cursor:'pointer', fontFamily:'IBM Plex Sans, sans-serif', fontSize:13 }}>← Back to site</button>
        </div>
      </motion.div>
    </div>
  )
