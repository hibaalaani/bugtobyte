...
export default function AuthPage({ setPage, initialMode = 'login' }: { setPage: (p: string) => void; initialMode?: Mode }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      if (!name.trim()) { toast.error(a.enterName); setLoading(false); return }
    ...
    if (mode === 'forgot') {
      ...
      if (error) { toast.error(error.message) } else { toast.success(a.resetSent); setMode('login') }
    }
    ...
    <div style={{ minHeight:'100vh', background:'#050A12', display:'grid', placeItems:'center', padding:'100px 24px 60px', position:'relative' }}>
      ...
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} style={{ width:'100%', maxWidth:440, position:'relative', zIndex:1 }}>
        ...
        <div style={{ textAlign:'center', marginBottom:40 }}>
          ...
          <h1 style={{ fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:28, color:'#F0EFE7', marginBottom:8 }}>{title}</h1>
          <p style={{ color:'rgba(240,239,231,.5)', fontFamily:'IBM Plex Sans, sans-serif', fontSize:15 }}>{subtitle}</p>
        </div>
          ...
          {mode !== 'forgot' && mode !== 'confirm-pending' && (
            <>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
                ...
                <span style={{ fontSize:12, color:'rgba(240,239,231,.35)', fontFamily:'IBM Plex Sans, sans-serif' }}>{a.orEmail}</span>
                <div style={{ flex:1, height:1, background:'rgba(255,255,255,.1)' }} />
              </div>
            </>
          )}
          ...
          {mode !== 'confirm-pending' && <form onSubmit={handleSubmit}>
            {mode === 'signup' && (
              <div style={{ marginBottom:18 }}>
                ...
                <div style={{ position:'relative' }}>
                  ...
                  <input value={name} onChange={e => setName(e.target.value)} placeholder={a.namePh} required style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'13px 14px 13px 42px', color:'#F0EFE7', fontFamily:'IBM Plex Sans, sans-serif', fontSize:15, outline:'none' }} onFocus={e => e.target.style.borderColor='rgba(0,255,135,.5)'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,.1)'} />
                </div>
              </div>
            )}

            <div style={{ marginBottom:18 }}>
              <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>{a.emailAddress}</label>
              <div style={{ position:'relative' }}>
                <Mail size={15} style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', color:'rgba(240,239,231,.35)' }} />
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={a.emailPh} required style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'13px 14px 13px 42px', color:'#F0EFE7', fontFamily:'IBM Plex Sans, sans-serif', fontSize:15, outline:'none' }} onFocus={e => e.target.style.borderColor='rgba(0,255,135,.5)'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,.1)'} />
              </div>
            </div>

            {mode !== 'forgot' && (
              <div style={{ marginBottom: mode === 'login' ? 8 : 28 }}>
                <label style={{ display:'block', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:600, fontSize:12, letterSpacing:'0.08em', color:'rgba(240,239,231,.6)', marginBottom:8, textTransform:'uppercase' }}>{a.password}</label>
                <div style={{ position:'relative' }}>
                  <Lock size={15} style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', color:'rgba(240,239,231,.35)' }} />
                  <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder={mode === 'signup' ? a.minChars : '••••••••'} required style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'13px 44px 13px 42px', color:'#F0EFE7', fontFamily:'IBM Plex Sans, sans-serif', fontSize:15, outline:'none' }} onFocus={e => e.target.style.borderColor='rgba(0,255,135,.5)'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,.1)'} />
            ...
            {mode === 'login' && (
              <div style={{ textAlign:'right', marginBottom:24 }}>
                <button type="button" onClick={() => setMode('forgot')} style={{ background:'none', border:'none', color:'rgba(0,255,135,.7)', cursor:'pointer', fontFamily:'IBM Plex Sans, sans-serif', fontSize:13 }}>{a.forgotPassword}</button>
              </div>
            )}

            <button type="submit" disabled={loading} style={{ width:'100%', background:'linear-gradient(135deg,#00FF87,#00D4AA)', color:'#050A12', border:'none', borderRadius:8, padding:'14px', fontFamily:'IBM Plex Sans, sans-serif', fontWeight:700, fontSize:15, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, opacity:loading?0.6:1 }}>
              {submitLabel}
        ...
        <div style={{ textAlign:'center', marginTop:24 }}>
          <button onClick={() => { setPage('home'); window.scrollTo({ top:0 }); }} style={{ background:'none', border:'none', color:'rgba(240,239,231,.3)', cursor:'pointer', fontFamily:'IBM Plex Sans, sans-serif', fontSize:13 }}>{a.backToSite}</button>
        </div>
      </motion.div>
    </div>
  )
